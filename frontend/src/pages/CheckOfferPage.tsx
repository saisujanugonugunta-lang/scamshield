...
export const CheckOfferPage: React.FC<CheckOfferPageProps> = ({
  ...
  const handleTriggerAnalysis = async () => {
    ...
    if (!text) {
      window.alert('Please paste a recruitment message before scanning.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/analyze`, {
        ...
        body: JSON.stringify({ message: text, source: activeTab }),
      });

      if (!response.ok) throw new Error('Analysis request failed');
      const result = await response.json();

      sessionStorage.setItem('scamshield:lastAnalysis', JSON.stringify({
        message: text,
        result,
      ...
      console.error(error);
      window.alert('Unable to reach the ScamShield analysis service. Please try again.');
    }
  };
